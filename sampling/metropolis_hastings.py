...
class MetropolisHastings:
    ...
    def gibbs_type(self, t: float = 1.0, proposal_distrib: Callable = None, trans_kern: Callable = None):
        """
        ...
        for t in range(self.T):
            for idx, _x in enumerate(self.x):
                for idx2, _xi in enumerate(_x):
                    ...
                    proposed_xi = _xi_0 + self.trans_kern(loc=self.loc, scale=scale[idx2])
                    _x_temp[idx2] = proposed_xi

                    num = self.pi(_x_temp)*self.proposal_distrib(_xi_0, proposed_xi, _scale=scale[idx2])
                    den = self.pi(_x)*self.proposal_distrib(proposed_xi, _xi_0, _scale=scale[idx2])
                    A = min(1, num/den)
                    if uniform.rvs() < A:
                        _x[idx2] = proposed_xi
                self.x[idx] = _x
